...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$Action = New-ScheduledTaskAction -Execute $Python -Argument ('\"' + $windowScript + '\" --duration 3300 --interval 1000 --snapshot-interval 60') -WorkingDirectory $ScriptDir; " ^
  ...
  "$Settings = New-ScheduledTaskSettingsSet -Hidden -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Hours 0) -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1); " ^
  "Register-ScheduledTask -TaskName 'KanshiWindowCapture' -Principal $Principal -Action $Action -Trigger $Trigger -Settings $Settings | Out-Null; " ^
